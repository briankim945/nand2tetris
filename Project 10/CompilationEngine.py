...
class CompilationEngine:
    ...
    def compileStatements(self, prefix="", terminator=['}']):
        self.writeLine(prefix + "<statements>")
        innerPrefix = prefix + "  "
        prior = None
        while not (self.jktk.tokenType() == "SYMBOL" and self.jktk.symbol(internal=False) in terminator):
            if self.jktk.symbol(internal=False) != prior:
                prior = self.jktk.symbol(internal=False)
    ...
    def compileExpression(self, prefix="", terminator=None):
        ...
        if terminator is not None:
            while not (self.jktk.tokenType() == "SYMBOL" and self.jktk.symbol(internal=False) in terminator):
                if self.jktk.tokenType() == "SYMBOL" and self.jktk.symbol(internal=False) == "(" and potentialFunction:
                    self.writeLine(innerPrefix + f"<symbol> {self.jktk.symbol()} </symbol>")
                    # (
                    self.jktk.advance()
                    # Expression list (arguments)
                    ...
                    self.writeLine(innerPrefix + f"<symbol> {self.jktk.symbol()} </symbol>")
                    potentialFunction = False
                elif self.jktk.tokenType() == "SYMBOL" and self.jktk.symbol(internal=False) in unary_operators:
                    ...
                    if self.jktk.tokenType() == "SYMBOL" and self.jktk.symbol(internal=False) in operators:
                        self.writeLine(innerPrefix + f"<symbol> {self.jktk.symbol()} </symbol>")
                    else:
                        self.compileTerm(prefix=innerPrefix, terminator=terminator)
                        continue
    ...
    def compileTerm(self, prefix="", terminator=None):
        ...
        una = False
        if terminator is not None:
            while not (self.jktk.tokenType() == "SYMBOL" and (self.jktk.symbol(internal=False) in terminator or self.jktk.symbol(internal=False) in operators)):
                if una:
                    self.compileTerm(prefix=innerPrefix, terminator=terminator)
                    potentialFunction = False
                    una = False
                    ...
                    if self.jktk.tokenType() == "SYMBOL":
                        ...
                        if potentialFunction and self.jktk.symbol(internal=False) == "(":
                            ...
                            self.jktk.advance()
                            self.compileExpression(prefix=innerPrefix, terminator=[')'])
                            # self.compileTerm(prefix=innerPrefix, terminator=[')'])
                            self.writeLine(innerPrefix + f"<symbol> {self.jktk.symbol()} </symbol>")
                        elif self.jktk.symbol(internal=False) == "[":
                            self.jktk.advance()
                            self.compileExpression(prefix=innerPrefix, terminator=[']'])
    ...
    def compileExpressionList(self, prefix="", terminator=None) -> int:
        ...
        if terminator is not None:
            while not (self.jktk.tokenType() == "SYMBOL" and self.jktk.symbol(internal=False) in terminator):
                if self.jktk.tokenType() == "SYMBOL" and self.jktk.symbol(internal=False) == ',':
                    self.writeLine(innerPrefix + f"<symbol> {self.jktk.symbol()} </symbol>")
                    self.jktk.advance()
                else:
                    self.compileExpression(prefix=innerPrefix, terminator=[',', ')'])
